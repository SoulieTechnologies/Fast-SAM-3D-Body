...
Uses the same `fast_sam_3d_body` environment as the main project. See the [root setup instructions](../README.md#environment):
...
Step 1a (`step1a_collect_fitted.py`) uses the MHR conversion tools, which require the [pixi](https://pixi.sh) environment from the [MHR repository](https://github.com/facebookresearch/MHR).

```bash
# Install pixi (if not already installed)
curl -fsSL https://pixi.sh/install.sh | bash

# Clone MHR and set up pixi environment (following official MHR instructions)
...
The MHR repository (cloned in Step 2 above) contains the barycentric mapping and SMPL model files under `tools/mhr_smpl_conversion/`.
