...
from visualize_skeleton_video import draw_skeleton, draw_option_b
...
def _emit_server(port):
    ...
    srv.listen(1)
    print(f"  keypoint emitter listening on tcp/{port} (waiting for IK client)...")
...
def _canonical_M(buf):
    ...
    """
    a = np.stack([f for f in buf if np.isfinite(f[[0, 5, 6, 9, 10, 13, 14]]).all()])
    if len(a) < 5:
        return None
...
def _receiver(port):
    ...
    srv.bind(("0.0.0.0", port))
    srv.listen(1)
    print(f"  camera receiver listening on tcp/{port} (waiting for Mac client)...")
    while not _STOP.is_set():
        conn, addr = srv.accept()
            ...
            while not _STOP.is_set():
                ...
                while len(buf) < ln:
                    ...
                    if not d:
                        raise ConnectionError
                    buf += d
                _RECV["idle"] = time.perf_counter() - _t_idle    # blocking-recv wait ≈ network gap
                _t_dec = time.perf_counter()
                img = cv2.imdecode(np.frombuffer(buf[:ln], np.uint8), cv2.IMREAD_COLOR)
...
def _select_person(outputs, prev_centroid, img_diag, w_pen=5.0):
    ...
    if not cands:
        return None
    key = ((lambda x: x[0]) if prev_centroid is None else
           (lambda x: x[0] / (1.0 + w_pen * np.linalg.norm(x[1] - prev_centroid) / img_diag)))
...
def _estimate_intrinsics(estimator, cap, n=10):
    ...
    idxs = np.linspace(0, max(total - 1, 1), n, dtype=int) if total > 1 else [0]
    Ks = []
    for i in idxs:
        if total > 1:
        ...
        if not ok:
            continue
        with torch.no_grad():
            K = estimator.fov_estimator.get_cam_intrinsics(
                cv2.cvtColor(f, cv2.COLOR_BGR2RGB)).squeeze().cpu().numpy()
        Ks.append(K)
    cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
    if not Ks:
        return None
    K = np.mean(Ks, axis=0)
    print(f"  intrinsics fx={K[0,0]:.0f} fy={K[1,1]:.0f} cx={K[0,2]:.0f} cy={K[1,2]:.0f}")
...
def inference_loop(args, estimator, cam_int):
    ...
    print(f"  inference: cam_int={'FIXED' if cam_int is not None else ('AUTO (MoGe2 once on 1st frame)' if auto_est else 'NONE -> per-frame MoGe2 SLOW')}")
    img_diag = None
    cen = None
    ema = None
    ...
    while not _STOP.is_set():
        ...
        if auto_est:                              # one-time MoGe2 calibration
            ...
            print(f"  AUTO intrinsics: fx={K[0,0]:.0f} fy={K[1,1]:.0f} "
                  f"cx={K[0,2]:.0f} cy={K[1,2]:.0f}", flush=True)
            auto_est = False
        ...
        _PROF["infer"] += dt
        if sel is not None:
            kp, yolo_kp, kp3d, cen = sel           # main subject only (ignores passers-by)
            ...
            if args.emit_port > 0:
                if M is None and np.isfinite(kp3d[[0, 5, 6, 9, 10, 13, 14]]).all():
                    ...
                    if len(align_buf) >= args.warmup:
                        ...
                        print(f"  gravity alignment locked after {len(align_buf)} frames", flush=True)
                if M is not None:
            ...
            if yolo_kp is not None:
                ...
                frame = draw_option_b(frame, yolo_kp, kp, frame.shape[1], frame.shape[0],
                                      args.hand_scale)
            else:
        ...
        ok2, buf = cv2.imencode(".jpg", frame, [cv2.IMWRITE_JPEG_QUALITY, args.mjpeg_quality])
        if ok2:
        ...
        if _n % 30 == 0:
            k = max(_PROF["n"], 1)
            print(f"  inference {ema:4.1f} FPS | per-frame: "
                  ...
                  f"recv-idle {1e3*_RECV['idle']:.1f}ms", flush=True)
...
_PAGE = (b"<html><head><title>SAM3D live</title></head>"
...
def main():
    ...
    p.add_argument("--intrinsics", default="", help="npz with 'K' (e.g. cam_params/cam0_intrinsics.npz) — no MoGe2")
    p.add_argument("--fx", type=float, default=0, help="fixed focal (0 = estimate with MoGe2)")
    p.add_argument("--fy", type=float, default=0)
    p.add_argument("--cx", type=float, default=0, help="principal point (0 = image centre)")
    ...
    p.add_argument("--emit-port", type=int, default=0,
    ...
    p.add_argument("--mjpeg-quality", type=int, default=70,
    ...
    if args.intrinsics:
        ...
        K = np.array([[args.fx, 0, cx], [0, args.fy or args.fx, cy], [0, 0, 1]], np.float32)
    have_fixed = K is not None
    cam_int = torch.tensor([K], dtype=torch.float32) if have_fixed else None
    if have_fixed:
        print(f"  fixed intrinsics: fx={K[0,0]:.0f} fy={K[1,1]:.0f} cx={K[0,2]:.0f} cy={K[1,2]:.0f}")
    ...
    estimator = setup_sam_3d_body(
        ...
        detector_name="yolo_pose", detector_model=det,   # exposes COCO-17 body keypoints (free)
        fov_name="" if have_fixed else "moge2", device="cuda",   # no MoGe2 when fixed
    )
    if not have_fixed:
        if args.recv_port > 0 or args.source.isdigit():
            print("  No fixed intrinsics → will AUTO-estimate once with MoGe2 on the "
    ...
    th = threading.Thread(target=inference_loop, args=(args, estimator, cam_int), daemon=True)
    th.start()

    server = ThreadingHTTPServer(("0.0.0.0", args.port), Handler)
    print(f"[3/3] Streaming on port {args.port}.")
    print(f"      Local:  ssh -L {args.port}:localhost:{args.port} theo@clear-antares.tailb614a0.ts.net")
