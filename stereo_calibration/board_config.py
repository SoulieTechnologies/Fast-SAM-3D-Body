...
def detect_charuco(gray, board, dictionary, min_corners=6):
    ...
    if hasattr(cv2.aruco, "CharucoDetector"):  # OpenCV >= 4.8
        ...
        ch_corners, ch_ids, mk_corners, mk_ids = _CHARUCO_DETECTOR.detectBoard(
            gray
        )
        if (
            (ch_ids is None or len(ch_ids) < min_corners)
            and mk_ids is not None
            and len(mk_ids) >= 8
        ):
            ...
            if i2 is not None and len(i2) >= min_corners:
                if not _LEGACY_ANNOUNCED:
                    print(
                        "[board_config] legacy-pattern ChArUco board detected "
