...
parser.add_argument(
    "--image", default="", help="diagnose a saved image instead"
)
parser.add_argument(
    "--frames", type=int, default=60, help="frames to scan for the best view"
)
...
if args.image:
    ...
    print(
        f"scanning {args.frames} frames — show the board to camera {args.cam}..."
    )
...
assert (
    n_mk >= 4
), f"only {n_mk} markers seen — check dictionary/lighting/distance"
ids = mk_ids.ravel()
...
sides = np.array(
    [
        np.linalg.norm(c.reshape(4, 2)[0] - c.reshape(4, 2)[1])
        for c in mk_corners
    ]
)
...
for name, sq in (
    ("diagonal-adjacent", square_px_diag),
    ("row-adjacent", square_px_row),
):
    print(f"marker/square ratio if {name}: {marker_px / sq:.2f}")
print(
    f"(configured ratio: {board_config.MARKER_SIZE / board_config.SQUARE_SIZE:.2f})"
)

# max marker id → number of squares on the board (markers fill half the squares)
n_squares_min = 2 * (ids.max() + 1)
print(
    f"max id {ids.max()} → the board has ~{n_squares_min} squares "
...
ratios = sorted(
    {
        round(board_config.MARKER_SIZE / board_config.SQUARE_SIZE, 2),
        0.7,
        0.75,
        0.6,
        0.8,
...
cand_dims = sorted(
    {
        (board_config.BOARD_COLS, board_config.BOARD_ROWS),
        (11, 8),
        (11, 3),
        (8, 11),
        (3, 11),
...
results = []
for (cols, rows), legacy, ratio in itertools.product(
    cand_dims, (False, True), ratios
):
    ...
    if n > 0:
        ...
        print(
            f"  {cols}x{rows}  legacy={legacy}  ratio={ratio:.2f}  ->  {n} corners"
        )

if not results:
    print(
        "  NOTHING interpolates — likely a different dictionary or a non-ChArUco board."
    )
else:
    ...
    print(
        "→ update board_config.py accordingly (SQUARE_SIZE = measured square in "
