...
def draw_skeleton(frame, pts2d, valid, img_w, img_h,
                  body_thickness=3, hand_thickness=2,
                  body_radius=5, hand_radius=3):
...
COCO_BONES = [
    (_c("nose"), _c("l_eye"), _BLUE), (_c("nose"), _c("r_eye"), _BLUE),
    (_c("l_eye"), _c("l_ear"), _BLUE), (_c("r_eye"), _c("r_ear"), _BLUE),
    (_c("l_sho"), _c("r_sho"), _BLUE),
    (_c("l_sho"), _c("l_hip"), _BLUE), (_c("r_sho"), _c("r_hip"), _BLUE),
    (_c("l_hip"), _c("r_hip"), _BLUE),
    (_c("l_sho"), _c("l_elb"), _GREEN), (_c("l_elb"), _c("l_wri"), _GREEN),
    (_c("r_sho"), _c("r_elb"), _ORANGE), (_c("r_elb"), _c("r_wri"), _ORANGE),
    (_c("l_hip"), _c("l_kne"), _GREEN), (_c("l_kne"), _c("l_ank"), _GREEN),
    (_c("r_hip"), _c("r_kne"), _ORANGE), (_c("r_kne"), _c("r_ank"), _ORANGE),
]
...
_HANDS = [
    {"sam_wrist": 41, "sam_range": range(21, 42), "yolo_wrist": _c("r_wri"), "yolo_elbow": _c("r_elb")},
    {"sam_wrist": 62, "sam_range": range(42, 63), "yolo_wrist": _c("l_wri"), "yolo_elbow": _c("l_elb")},
]
...
def compute_hand_scale(joints, yolo_kp):
    ...
    for f in range(min(len(joints), len(yolo_kp))):
        s = joints[f, :, :2]
        y = yolo_kp[f]
        y_ms = (y[5] + y[6]) / 2; y_mh = (y[11] + y[12]) / 2
        s_ms = (s[5] + s[6]) / 2; s_mh = (s[9]  + s[10]) / 2
        if np.isnan([y_ms, y_mh, s_ms, s_mh]).any():
            continue
        sl = np.linalg.norm(s_ms - s_mh); yl = np.linalg.norm(y_ms - y_mh)
...
def draw_option_b(frame, yolo_kp, sam_pts2d, img_w, img_h, hand_scale=1.0):
    ...
    def ok(p):
        return not np.isnan(p).any() and 0 <= p[0] < img_w and 0 <= p[1] < img_h
    ...
    for i in range(17):
        p = yolo_kp[i]
        if ok(p):
            cv2.circle(overlay, (int(p[0]), int(p[1])), 5, (255, 255, 255), -1, cv2.LINE_AA)
            cv2.circle(overlay, (int(p[0]), int(p[1])), 5, (0, 0, 0), 1, cv2.LINE_AA)
    for i in range(21, 63):
        p = hand_pts[i]
        if ok(p):
            cv2.circle(overlay, (int(p[0]), int(p[1])), 3, (255, 255, 255), -1, cv2.LINE_AA)
            cv2.circle(overlay, (int(p[0]), int(p[1])), 3, (0, 0, 0), 1, cv2.LINE_AA)
...
def reproject(joints_3d, fx, fy, cx, cy):
    ...
    z = joints_3d[:, :, 2]
    valid = z > 1e-4
    out[:, :, 0] = np.where(valid, joints_3d[:, :, 0] / np.where(valid, z, 1) * fx + cx, np.nan)
    out[:, :, 1] = np.where(valid, joints_3d[:, :, 1] / np.where(valid, z, 1) * fy + cy, np.nan)
    return out


def main():
    p = argparse.ArgumentParser(description="Overlay Goliath skeleton on video")
    p.add_argument("--npy",    required=True, help="(T, 70, 2) joints_2d.npy  OR  (T, 70, 3) joints_3d.npy")
    p.add_argument("--video",  required=True, help="Original video path")
    p.add_argument("--output", default="skeleton_overlay.mp4")
    p.add_argument("--scale", type=float, default=1.0, help="Scale skeleton around its centroid (e.g. 1.25)")
    p.add_argument("--yolo", default="", help="joints_yolo_2d.npy — Option B: draw pixel-accurate YOLO body + SAM3D hands")
    p.add_argument("--fx", type=float, default=0, help="Focal length x — required when --npy is joints_3d.npy")
    ...
    assert joints.ndim == 3 and joints.shape[1] == 70, "Expected shape (T, 70, 2 or 3)"

    yolo_kp = None
    hand_scale = 1.0
    if args.yolo:
        yolo_kp = np.load(args.yolo)
        assert yolo_kp.ndim == 3 and yolo_kp.shape[1] == 17, "Expected YOLO shape (T, 17, 2)"
        hand_scale = compute_hand_scale(joints, yolo_kp)
        print(f"Option B: YOLO body + SAM3D hands  {yolo_kp.shape}  hand_scale={hand_scale:.3f}")

    if joints.shape[2] == 3:
        # 3D input — reproject with provided intrinsics
        if args.fx <= 0:
            raise ValueError("--npy contains 3D joints: please provide --fx/--fy/--cx/--cy")
        ...
        print(f"Reprojected 3D→2D: fx={args.fx} fy={fy} cx={cx:.0f} cy={cy:.0f}")
    ...
    for frame_idx in range(total):
        ...
        if frame_idx < n_joints:
            ...
            if yolo_kp is not None and frame_idx < len(yolo_kp):
                # Option B — pixel-accurate YOLO body + SAM3D hands
                frame = draw_option_b(frame, yolo_kp[frame_idx], pts2d, vid_w, vid_h, hand_scale)
            else:
                valid = ~np.isnan(pts2d).any(axis=1)
                if args.scale != 1.0 and valid.any():
                    center = pts2d[valid].mean(axis=0)
                    pts2d[valid] = center + (pts2d[valid] - center) * args.scale
