...
def _run_view(estimator, frame_bgr, cam_int, body_only, prev_centroid, img_diag):
...
def _triangulate_frame(tri, p0, p1):
    ...
    if valid.sum() == 0:
        return out
    pts3d = tri.triangulate(p0[valid].astype(np.float32), p1[valid].astype(np.float32))
    out[valid] = pts3d
    return out
...
def _b2_hands_global(tri_3d, mono_3d, scales):
    ...
    """
    out = tri_3d.copy()
    for wri, elb, rng, key in [(R_WRI, R_ELB, R_HAND, "R"), (L_WRI, L_ELB, L_HAND, "L")]:
        w_t, e_t = tri_3d[wri], tri_3d[elb]
        w_m, e_m = mono_3d[wri], mono_3d[elb]
        if not np.isfinite([w_t, e_t, w_m, e_m]).all():
            continue
        s = scales[key]
        Rrot = _rot_from_vectors(w_m - e_m, w_t - e_t)   # align forearm direction
        for i in rng:
...
def _report_metric(name, j3d):
    ...
    torso = np.linalg.norm(mid(j3d, L_SHO, R_SHO) - mid(j3d, L_HIP, R_HIP), axis=1)
    luarm = _seg(j3d, L_SHO, L_ELB)
    lthigh = _seg(j3d, L_HIP, L_KNE)
    f = lambda x: np.nanmedian(x)
    print(f"  {name:14s} torso={f(torso)*100:5.1f}cm  L-upperarm={f(luarm)*100:5.1f}cm  "
...
def canonical_align(tri3d):
    ...
    if not np.isfinite(up).all() or np.linalg.norm(up) < 1e-6:
        return tri3d.copy(), np.eye(3, dtype=np.float32)   # can't align → passthrough
...
def main(args):
    ...
    print(f"[1/4] Loaded stereo calib: img_size={tri.img_size}  baseline≈{np.linalg.norm(tri.T)*100:.1f}cm")
    ...
    total = min(n0, n1)
    print(f"[3/4] cam0={n0} frames, cam1={n1} frames → processing {total} synced frames")
    if n0 != n1:
        print(f"  WARNING: frame counts differ ({n0} vs {n1}); assuming aligned from frame 0.")

    img_diag = float(np.hypot(*tri.img_size))   # for the tracking distance penalty
    ...
    for idx in range(total):
        ...
        p0, m0, cen0 = _run_view(estimator, f0, cam_int0, args.body_only, cen0, img_diag)
        p1, _, cen1 = _run_view(estimator, f1, cam_int1, args.body_only, cen1, img_diag)
        times.append(time.perf_counter() - t0)
        kp2d_0.append(p0); kp2d_1.append(p1); kp3d_mono0.append(m0)
        if idx % 20 == 0:
            avg = 1.0 / np.mean(times[-20:]) if times else 0
            print(f"  frame {idx}/{total}  {1.0/times[-1]:.1f} FPS (avg {avg:.1f})")
    cap0.release(); cap1.release()

    kp2d_0 = np.stack(kp2d_0); kp2d_1 = np.stack(kp2d_1); kp3d_mono0 = np.stack(kp3d_mono0)
    ...
    for f in range(T):
        d3 = _triangulate_frame(tri, kp2d_0[f], kp2d_1[f])      # rectified cam0 frame
    ...
    if args.hands == "b2":
        print(f"  global forearm scale: right={scales['R']:.3f}  left={scales['L']:.3f}")
    ...
    dropped = 0
    for f in range(T):
        d3 = _b2_hands_global(raw[f], kp3d_mono0[f], scales) if args.hands == "b2" else raw[f].copy()
        v = np.isfinite(d3).all(1)
        if not v.any():
            continue
        pts = cv2.projectPoints(d3[v].astype(np.float64), z3, z3, tri.K1, tri.D1)[0].reshape(-1, 2)
    ...
    # ── Save ────────────────────────────────────────────────────────────────
    out = args.output_dir
    np.save(os.path.join(out, "joints_3d_tri.npy"), tri_3d)          # cam0 frame (for overlay)
    ...
    print(f"Kept {kept}/{T} frames   dropped {dropped} (reproj gate > {args.reproj_gate}px → person mismatch)")
    print(f"Triangulation reprojection error (cam0, kept): "
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Two-camera SAM3D extraction + triangulation")
    p.add_argument("--cam0", required=True, help="cam0 video")
    p.add_argument("--cam1", required=True, help="cam1 video")
    p.add_argument("--stereo", required=True, help="stereo_params.npz (with K1,D1,K2,D2,R,T,P1,P2,R1,R2)")
    ...
    p.add_argument("--hands", choices=["tri", "b2"], default="b2",
                   help="tri: triangulate fingers; b2: cam0 mono hand scaled+anchored to triangulated wrist")
    p.add_argument("--depth-min", type=float, default=0.2, help="reject triangulated points closer than this (m)")
    p.add_argument("--depth-max", type=float, default=4.0, help="reject triangulated points farther than this (m)")
