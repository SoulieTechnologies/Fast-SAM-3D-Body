...
def _rx_thread(sock):
    ...
    while True:
        ...
        if msg is not None:
            ...
            _RX["kp"] = (
                np.frombuffer(msg[4:], dtype=np.float32).reshape(70, 3).copy()
            )
    ...
    while True:
        try:
            ...
            if time.time() - t0 > timeout:
                raise TimeoutError(
                    f"emit server {host}:{port} not up after {timeout}s"
                )
...
class RerunMirror:
    ...
    def log(self, frame_n, est, gt, ik_ms):
        ...
        gt_pts = np.array(
            [gt[k] for k in self.keys if k in gt and np.isfinite(gt[k]).all()]
        )
        if len(gt_pts):
            rr.log(
                "retarget/target",
                rr.Points3D(
                    positions=gt_pts, radii=0.006, colors=[255, 60, 60]
                ),
            )
...
def main():
    p = argparse.ArgumentParser(
        description="Live ACADOS IK + Rerun from streamed SAM3D 3D"
    )
    ...
    p.add_argument(
        "--augmenter-path",
        default=str(
            Path(__file__).resolve().parent.parent / "augmentation_model"
        ),
    )
    p.add_argument(
        "--warmup",
        type=int,
        default=30,
        help="frames to collect before starting",
    )
    args = p.parse_args()
    ...
    q_init = RT_IK(
        human_model, fd0, pin.neutral(human_model), wkeys, args.dt, omega
    ).solve_ik_sample_casadi()
    ik = AcadosMPCIKSolver(
        pin_model=human_model,
        keys_to_track=valid_keys,
        N=args.N,
        dt=args.dt,
    ...
    window = deque(
        [_frame_dict(f) for f in warm_arr[-args.N :]], maxlen=args.N
    )
    ...
    while True:
        ...
        for mk in valid_keys:
            est[mk] = np.array(
                human_data.oMf[human_model.getFrameId(mk)].translation
            )
        ...
        if time.time() - t_report > 2.0:
            print(
                f"  IK {solved / (time.time() - t_report):.1f} solves/s "
                f"({ik_ms:.0f} ms last)",
                flush=True,
            )
            solved = 0
