...
if _WORKER:
    ...
    for k, v in {
        ...
        "LAYER_DTYPE": "fp32",
        "GPU_HAND_PREP": "1",
        "INTERM_PRED_INTERVAL": "999",
        "KEYPOINT_PROMPT_INTERM_INTERVAL": "999",
        "COMPILE_MODE": "default",
        "MHR_USE_CUDA_GRAPH": "0",
        "TRT_BACKBONE_PATH": "/home/users/theo/code/checkpoints/sam-3d-body-dinov3/backbone_trt/backbone_dinov3_fp16.engine",
    }.items():
...
def run_worker(args):
    ...
    cam_int = (
        torch.tensor(K[None], dtype=torch.float32) if K is not None else None
    )
    ...
    print(
        f"[worker gpu{args.gpu}] {len(kp)} frames, SAM3D compute {1.0/(t_proc/max(len(kp),1)):.1f} FPS"
    )
...
def run_orchestrator(args):
    ...
    k0 = os.path.join(args.output_dir, "_K0.npz")
    np.savez(k0, K=tri.K1)
    k1 = os.path.join(args.output_dir, "_K1.npz")
    np.savez(k1, K=tri.K2)
    ...
    print(
        f"[orchestrator] launching 2 workers — cam0→gpu{args.gpu0}, cam1→gpu{args.gpu1} (parallel)"
    )
    t0 = time.perf_counter()
    ...
    print(
        f"[orchestrator] parallel extraction wall-clock: {wall:.1f}s → {T/wall:.1f} FPS for the stereo pair"
    )
    ...
    for f in range(T):
        ...
        if v.sum():
            X = tri.triangulate(
                a[v].astype(np.float32), b[v].astype(np.float32)
            )
            d3[v] = (tri.R1.T @ X.T).T
        bad = (
        ...
        rp[vv] = cv2.projectPoints(
            d3[vv].astype(np.float64), z3, z3, tri.K1, tri.D1
        )[0].reshape(-1, 2)
        bodyv = vv[:21]
        if (
            bodyv.any()
            and np.median(
                np.linalg.norm(rp[:21][bodyv] - a[:21][bodyv], axis=1)
            )
            > args.reproj_gate
        ):
            dropped += 1
    ...
    if np.isfinite(up).all() and np.linalg.norm(up) > 1e-6:
        ...
        world = (
            (M @ tri3d.reshape(-1, 3).T)
            .T.reshape(tri3d.shape)
            .astype(np.float32)
        )
    ...
    print(
        f"Reproj (cam0): median={np.nanmedian(rerr):.1f}px  p90={np.nanpercentile(rerr,90):.1f}px"
    )
    print(
        f"Output: {args.output_dir}/joints_3d_tri.npy  ({T},70,3) Goliath metric"
    )
    print("=" * 60)
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Dual-GPU two-camera extraction + triangulation"
    )
    p.add_argument("--worker", action="store_true", help=argparse.SUPPRESS)
    # worker args
    p.add_argument("--video")
    p.add_argument("--gpu", type=int, default=0)
    p.add_argument("--out2d")
    p.add_argument("--intrinsics", default="")
    # orchestrator args
    p.add_argument("--cam0")
    p.add_argument("--cam1")
    p.add_argument("--stereo")
    p.add_argument("--output_dir", default="./output_dualgpu")
    p.add_argument("--gpu0", type=int, default=7)
