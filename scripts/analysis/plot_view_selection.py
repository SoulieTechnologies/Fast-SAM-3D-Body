...
def main():
    ...
    ap.add_argument(
        "run_dir", help="cosmik_hand_demo output dir (or the .npy)"
    )
    ap.add_argument("--show", action="store_true")
    args = ap.parse_args()
    path = (
        args.run_dir
        if args.run_dir.endswith(".npy")
    ...
    act = {
        "R": np.isfinite(h[:, :, :21]).all(3).sum(2) > 10,  # (T, ncam)
    ...
    for hand, a in act.items():
        ...
        n_active = a.sum(1)
        sw = (
            (a[1:] != a[:-1]).any(1) & (n_active[1:] > 0) & (n_active[:-1] > 0)
        )
        print(
            f"  {hand}: "
            + "  ".join(f"cam{v} {use[v]:5.1f}%" for v in range(ncam))
