...
def load(path):
    ...
    if d.shape[1] != len(COLS):
        raise SystemExit(
            f"{path}: {d.shape[1]} columns, expected {len(COLS)} "
            f"(old format?)"
        )
...
def stats_table(runs):
    ...
    for path, d in runs.items():
        ...
        print(
            f"{name:{w}} ({n} hand iters, {dur:.0f}s, hand {hand_hz:.1f} Hz)"
        )
        for stat, fn in (
            ("mean", np.mean),
            ("median", np.median),
        ...
        if (sr >= 0).any():
            print(
                f"{'':{w}} stereo jts R {np.mean(sr[sr >= 0]):.1f}/21 "
...
def plot(runs, show):
    ...
    for path, d in runs.items():
        ...
        if len(t) > 1:
            ...
            ax.plot(
                t[1:],
                np.clip(1.0 / np.maximum(dt, 1e-3), 0, 60),
                lw=0.6,
                alpha=0.6,
                label=f"{name} hand Hz (inst)",
            )
        ax.set_ylabel("Hz")
...
def main():
    ...
    p.add_argument(
        "logs", nargs="+", help="timing.log path(s) — several to compare"
    )
    p.add_argument(
        "--show",
        action="store_true",
